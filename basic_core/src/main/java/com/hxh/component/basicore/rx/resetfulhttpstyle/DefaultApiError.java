...
public class DefaultApiError implements IApiError{
    ...
    public DefaultApiError getApiError(Throwable throwable)
    {
        try {
            if (throwable instanceof HttpException) {
                HttpException httpException = (HttpException) throwable;
                return JSON.parseObject(httpException.response().errorBody().string(), DefaultApiError.class);
            }
        }catch (Exception e)
        {
    ...
    @Override
    public String checResponseBodyContainErrorBodyRetrunErrorMessage(Response body) {
        okhttp3.ResponseBody errorBody = body.errorBody();
       if(null != errorBody)
       {
           try {
               return  body.errorBody().string();
           } catch (IOException e) {
               e.printStackTrace();
           }
       }
    ...
    public IApiError checkResponseBodyContainErrorBodyReturnErrorBody(Response body) {
        okhttp3.ResponseBody resbody = body.errorBody();
        if(null != resbody)
        {
            try {
                String bodystr = body.errorBody().string();
                if(!Utils.Text.isEmpty(body))
                {
                    return JSON.parseObject(bodystr, DefaultApiError.class);
                }
            } catch (IOException e) {
                e.printStackTrace();
            }
    ...
    {
        return getApiError(th).getErrorCode();
    }

    @Override
    public String getErrorMessage(Throwable th) {
        DefaultApiError api = getApiError(th);
        if(null != api)return api.getErrorMessage();
       else return "";
