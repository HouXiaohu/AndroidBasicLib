...
public class IntentDelegate<T extends Parcelable> implements IIntentRelated<T> {
    ...
    @Override
    public ArrayList<T> getExtra_ParceableArray(String key)
    {
        if (null != mDefaultBundle)
        {
            ArrayList<T> bean = mDefaultBundle.getParcelableArrayList(key);
            if(null != bean)
            {
                return bean;
            }
    ...
    @Override
    public ArrayList<T> getExtra_ParceableArray() {
        if (null != mDefaultBundle)
        {
            ArrayList<T> bean = mDefaultBundle.getParcelableArrayList(FROM_OBJ_TAG);
            if(null != bean)
            {
                return bean;
            }
