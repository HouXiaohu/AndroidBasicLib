...
import com.hxh.component.basicore.component.mvp.delegate.interfaces.IAdapterRelated;
...
public class IAdapterDelegate<T, H extends RecyclerView.ViewHolder> implements IAdapterRelated<T, H> {
    ...
    public boolean isEmpty(TextView tv, String msg) {
        return viewRelatedDelegate.isEmpty(tv, msg);
    }
        ...
        if (null != mDatas) {
            if (mDatas.contains(data)) {
                ...
                if (position != mDatas.size()) { // 如果移除的是最后一个，忽略
                    mAdapter.notifyItemChanged(position, mDatas.size() - position);
                }
